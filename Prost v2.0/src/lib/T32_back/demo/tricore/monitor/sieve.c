/* *********************************************************************** */
/* Sieve Demo for TriCore ROM Monitor.                                     */
/*                                                                         */
/* Adaption of the Lauterbach sieve() demo for the TriCore ROM Monitor.    */
/* The changes consist of some additional interface functions as hooks for */
/* the Monitor and an additional initialization in main().                 */
/*                                                                         */
/* See montest.c for a less complex example using the ROM Monitor and for  */
/* additional information on how to design the interface functions between */
/* application and ROM Monitor.                                            */
/*                                                                         */
/* Version Information                                                     */
...
void Monitor_Init(void)
{
	...
	__asm("rslcx");
	__asm("nop");    /* workaround CPU_TC.069 */
}
...
void __trap(2) Trap_InstructionError(void)
{
	...
	switch (TIN) {
		case 1:    /* IOPC - Illegal Opcode                */
			...
			__asm("jl Monitor_EntryBreak");
			break;
		case 2:    /* UOPC - Unimpelented Opcode           */
...
/* *********************************************************************** */
/* Standard sieve demo as used for all TriCore demos.                      */
/* Note the initial init and interrupt enabling at the beginning of main.  */
...
inline void func1i( int * intptr )		/* static function */
{
	(*intptr)++;
}

static void func1c( char * intptr )		/* static function */
{
	(*intptr)++;
}

static void func1l( long * intptr )		/* static function */
{
	(*intptr)++;
}
...
void func2(void)
{
	...
	func1i( &fstatic );	/* to force fstatic as static-scope */
...
void func2a(void)
{
	...
	func1c( &autovar );	/* to force autovar as stack-scope */
...
void func2b(void)
{
	...
	func1l( &autovar );	/* to force autovar as stack-scope */
...
void func47(void)			/* force real stackframe */
{
	int autovar;
	register int regvar;
	static int fstatic = 44;	/* initialized static variable */
...
main(void)
{
	...
	/* ------------------------------------------------------------------- */
	/* additional code for ROM Monitor */
