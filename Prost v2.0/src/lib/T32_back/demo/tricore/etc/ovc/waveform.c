...
/** resulting histogramm */
SHistogram TC3_result;



void TC3_mix(SInput* in) {
	int i;
	for(i=0;i<SIZE;++i)
	{
		TC3_mixed[i] = (in->a[i]-in->b[i]);
	}
}
...
int TC3_main() {
	...
	while(1){
		nIxProcess = nProcessNext;
		nNowProcessing[3] = nIxProcess;
		TC3_mix(&inputBufs[nIxProcess]);
		HIST_histogram(&TC3_result, TC3_mixed);
	}
}
...
void TC4_mix(SInput* in) {
	...
	for(i=0;i<SIZE;++i)
	{
		TC4_mixed[i] = __abs((in->a[i]-in->b[i]));
	}
}
...
/** resulting histogramm */
SHistogram TC5_result;



void TC5_mix(SInput* in) {
	int i;
	TC5_mixed[0] = (in->a[0]+in->b[0])/2;
	TC5_mixed[1] = (in->a[0]+in->b[0])/2;
	for(i=2;i<SIZE;++i)
	{
		TC5_mixed[i] = ((in->a[i]+in->b[i])+TC5_mixed[i-1]+TC5_mixed[i-2])/4;
	}
}
...
int TC5_main() {
	...
	while(1){
		nIxProcess = nProcessNext;
		nNowProcessing[5] = nIxProcess;
		TC5_mix(&inputBufs[nIxProcess]);
		HIST_histogram(&TC5_result, TC5_mixed);
	}
}
...
const STC0_mix_params __far TC0_mix_params = {100,100};
