...
volatile flags_t __share nFlagsUncached  __at(0xB0000000) ;
...
int main0(void)
{
	...
	nFlagsUncached.b.Core2Reading = 0;
	
	int k = 0;
	while(1)
	{
		
		while((nFlagsUncached.b.Core1Reading||nFlagsUncached.b.Core2Reading));
		
		nFlagsUncached.b.Core0Writing = 1;
		fill(inputA,32,k*3,1024*nBase);
		fill(inputB,64,k*5,1024*nBase);
		int i = SIZE;                                                 /** write back data to ensure that other cores read the right values */
		int *pa = inputA, *pb = inputB;
		for(int i = 0; i < SIZE; i++) {
			pa = (int*)__cacheawi_bo_post_inc((unsigned char*)pa);
			pb = (int*)__cacheawi_bo_post_inc((unsigned char*)pb);
		}
...
int main1(void)
{
	while(1)
	{
		...
		int* p = inputA;
		while(p < inputA + SIZE)									/** invalidate cache to get updated array in next iteration*/
			p = (int*)__cacheawi_bo_post_inc((unsigned char*)p);
...
int main2(void)
{
	...
	{
		while(nFlagsUncached.b.Core0Writing);
		nFlagsUncached.b.Core2Reading = 1;
		mix2();
		nFlagsUncached.b.Core2Reading = 0;
		int* p = inputA;
		while(p < inputA + SIZE)
