...
{
	//volatile unsigned char * pout = (unsigned char *) 0xa1018000;
...
{
	//volatile unsigned char * pin = (unsigned char *) 0xa1018100;
...
int main()
{
	...
	while (1)
	{
		...
		if (ch == 'r' || ch == 'R')
		{
			handle = TerminalOpenFile(inFile,0);
			if (handle == -1) {
		...
		if (ch == 'w' || ch == 'W')
		{
			handle = TerminalOpenFile(outFile,4);
			if (handle == -1) {
				TerminalPuts( "failed to open file\r\n" );
				continue;
			}
