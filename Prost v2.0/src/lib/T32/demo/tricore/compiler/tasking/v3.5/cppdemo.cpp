/**
 * @file cppdemo.cpp
 * @brief Example for TriCore TASKING C++ compiler support.
 *
 * This example was compiled for running in the TRACE32 TriCore simulator
 * using PBI=SIM. The memory map may not match to a real target.
 *
 * $Id: cppdemo.cpp 5585 2013-06-27 16:46:25Z mobermeir $
 *
 * (c) Lauterbach GmbH
 */
...
enum enumtyp {
	enum1, enum2, enum4 = 4, enum7 = 7, enum8, enumx = -1
} enumvar;
...
void Tree::set_branch_impl(int n) /* implicit use of this */
{
	branch_cnt = n;
}
...
/* sieve demo */
#define SIZE 18        //!< Odd numbers to check for being a prime.
char flags[SIZE+1];    //!< Result array (only odd numbers). Prime numbers are marked 1.

/**
 * Sieve of Erathostenes.
  * Compute prime numbers between 3 and (2*SIZE)+1.
 * @return Number of found prime numbers.
 */
int sieve()
{
...
int main() {
	...
	do {
		...
		start: j = func5((int) j, (char) 2, (long) 3);
