...
static unsigned int ITM_MSGCNT = 0;
static unsigned int ITM_MSGWAIT = 0;
static unsigned int ITM_WAIT = 0;

#define ITM_TRACE_D32(_channel_,_data_) { \
	volatile unsigned int *_ch_=ITM_BASE+(_channel_); \
	if (*_ch_ == 0) {ITM_MSGWAIT++; do{ITM_WAIT++;}while(*_ch_ == 0);} \
	*_ch_ = (_data_); \
	ITM_MSGCNT++;\
}
...
{
	unsigned int addr;
	addr=(unsigned int)this_fn;
	addr&=~0x1;
	ITM_TRACE_D32(5,addr);
}

void __cyg_profile_func_exit(void *this_fn, void *call_site)
{
	unsigned int addr;
	addr=(unsigned int)call_site;
	addr&=~0x1;
	ITM_TRACE_D32(5,addr);
}
