...
#ifdef LINUX
int mypipe_create_read(mypipe_t *pipe,char *pipe_name,int wait)
{
	int accmode;
	mypipe_t p;
	/* Create a named pipe for reading data from it */
	mkfifo(pipe_name,0660);
	/* open pipe (with or without waiting for connection) */
	if (!wait)
...
FILE * mypipe_to_stream_read(mypipe_t pipe)
{
	return fdopen(pipe,"rb");
}

FILE * mypipe_to_stream_write(mypipe_t pipe)
{
	return fdopen(pipe,"wb");
}
...
int mypipe_create_read(mypipe_t *pipe,char *pipe_name,int wait)
{
	...
	if (wait)
	{
	    /* Wait until something connects to this pipe */
	    if (ConnectNamedPipe(p,NULL)==0)
		return -2;
	}
	*pipe=p;
	return 0;
...
{
	int pipefd;
	pipefd=_open_osfhandle((intptr_t)pipe,0);
	if (pipefd<0)
		return (FILE *)0;
	return _fdopen(pipefd,"rb");
}
