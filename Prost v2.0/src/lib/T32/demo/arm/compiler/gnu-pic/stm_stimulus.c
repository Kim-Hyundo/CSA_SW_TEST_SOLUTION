...
#define STP_TRACE_D32(_pstmbase_,_channel_,_stimuliport_,_type_,_data_)   ((*(volatile unsigned long  *)(_pstmbase_+0x1000*_channel_+_stimuliport_*0x100+_type_))=(unsigned long )(_data_))
...
void stmPrint(unsigned long nChannel, unsigned long nPort, char * pString)
{
  if (pString[0]) {
    ...
    // the last STM packet is generated with timestamp -> flush TRACE32 PrintfTrace
    STP_TRACE_D8(pStimulusBase, nChannel, nPort, STP_G_DTS, *pString);
  }
}
