...
void itmPrint(void* pStimulus, char * pString, int bDelimiter)
{
  int i;
  unsigned long tmp;
  while (*pString) {
...
int main()
{
  ...
  while(TRUE)
  {
    ...
    if (pPrintfString[0]!=0x0) {
      itmPrint(pStimulusBase, pPrintfString, TRUE);
    }
    else if (pTerminalString[0]!=0x0) {
      itmPrint(pStimulusBase, pTerminalString, FALSE);
    }
