/* $Id: t32report.js 13280 2018-10-08 13:02:26Z hlohn $ */
...
function addAddrForTotal(index) {  /* add address links */
	var totalrange = $("td.addr", this).first();
	if (totalrange.text())
		return;
	var re = /([a-z]+:)?(0x)([0-9a-f]+)--(0x)?([0-9a-f]+)/i;
	var memClassStr, fromStr, toStr;
	var sibling = $(this).siblings();
	var arr;
	for (var i = 0; i < sibling.length; i++){
		var addr = sibling.eq(i).find("td.addr").text();
		if ((arr = re.exec(addr)) !== null) {
			memClassStr = arr[1];
			fromStr = arr[3];
			break;
		}
	...
	if (memClassStr && fromStr && toStr){
		totalrange.text(memClassStr + fromStr + '--' + toStr);
	}
}
...
function beautifyAdresses() {
	...
	$('td.addr, span.addr').each(function(index) {
		var re = /:(0x)?([0-9a-f]+--)?(0x)?0*([0-9a-f]+)/i;
	...
	$('td.addr, span.addr').each(function(index) {
		var re = /([a-z]+:)(0x)?([0-9a-f]+)(--)?(0x)?0*([0-9a-f]+)?/i;
		var addrStr = $(this).text();
		var arr;
		if ((arr = re.exec(addrStr)) !== null) {
...
function addFileLinks(){
	if ($('#t32-ListModule').length && $('#t32-list').length == 0){ /* COVerage.ListModule, but not via export_asm_module ? */
		$('#t32-ListModule td.tree').each(function(index) {
			...
			if ((arr = re.exec($(this).text())) !== null) {
				...
				if (prog && module && module!="Global"){
					var filename = fix_filename(prog) + '/' + fix_filename(module) +"/index" + gFileExt;
...
function gotoAddress(addr){
	...
	var src = re.exec(addr);
	if (src && src[3]){
		var list = $('#t32-list tr.asm td.addr');
		for (var i = 0; i < list.length; i++){
			var $td = list.eq(i);
			var dst = re.exec($td.text());
			if (dst && src[3] == dst[3]){
				if (getListMode() == "HLL")
					setListMode("MIX")
...
$(document).ready(function(){
	...
	/* add potentially missing addresses in summary rows */
	$('table.t32-coverage tr.total').each(addAddrForTotal);

	/* pad adresses (and remove pontetial "0x" prefix) */
	beautifyAdresses();
