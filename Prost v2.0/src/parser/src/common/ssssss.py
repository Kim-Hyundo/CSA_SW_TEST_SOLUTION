    ...
    if die.tag == "DW_TAG_subprogram":
        ...
        if type_val in struct_map.keys():
            ...
            for member in members.keys():
                ...
                addVariableInMap(global_flag
                                 , global_map, variables, offset_var, var_name, type_val, line,
                                 global_access_map, struct_offset)

        else:
            addVariableInMap(global_flag, global_map, variables, offset_var, var_name, type_val, line,
                             global_access_map, 0)
...
def addVariableInMap(global_flag, global_map, variables, offset_var, var_name, type_val, line, global_access_map,
                     struct_offset):

    if (offset_var == '' or not offset_var.isdigit()):
        return
...
process_file(r"D:\T32_202002\demo\arm\hardware\s32k\s32k144\sieve_flash_thumb_ii_v7m.elf")