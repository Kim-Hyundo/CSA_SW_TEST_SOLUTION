...
def code_coverage_parser(dic_project_info, sourcemetric):
    log.set_log("REPORT", "code_coverage_parser Processing...", log.Logtype_PFF)
    list_coverage_rawdata = []
    dic_coverage_rawdata = {}

    if not os.path.isdir(dic_project_info['Common_Path']['WorkingProject_Path'] + "\\Debug\\txt\\" + sourcemetric):
        log.set_log("REPORT", "Coverage txt folder path is not exist", log.Logtype_ERR)
        return False

    scenario_folderlist = os.listdir(dic_project_info['Common_Path']['WorkingProject_Path'] + "\\Debug\\txt\\" + sourcemetric)
    ...
    for scenario_name in scenario_folderlist:
        ...
        file_list = os.listdir(dic_project_info['Common_Path']['WorkingProject_Path'] + "\\Debug\\txt\\" + sourcemetric + "\\" + scenario_name + "\\cov")

        if not file_list:
            log.set_log("REPORT", "folder is empty", log.Logtype_ERR)
            return False

        if not os.path.isdir(dic_project_info['Common_Path']['WorkingProject_Path'] + "\\Debug\\dic\\" + sourcemetric + "\\cov"):
            os.makedirs(dic_project_info['Common_Path']['WorkingProject_Path'] + "\\Debug\\dic\\" + sourcemetric + "\\cov")

        for file_name in file_list:

            load_file_path = dic_project_info['Common_Path']['WorkingProject_Path'] + "\\Debug\\txt\\" + sourcemetric + "\\" + scenario_name + "\\cov\\" + file_name
            save_file_path = dic_project_info['Common_Path']['WorkingProject_Path'] + "\\Debug\\dic\\" + sourcemetric + "\\cov\\" + file_name
