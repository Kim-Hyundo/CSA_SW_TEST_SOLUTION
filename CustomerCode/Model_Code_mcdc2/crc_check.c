...
/**********************************************************
	CRC-16 test sample
**********************************************************/
int crc_check(unsigned char *ptr)
{
	unsigned short	crc, check, data;
	unsigned char	in_frame[20];
	unsigned char	out_frame[20];
	...
	check = update_crc(0, out_frame, 6);

	if (*ptr == out_frame[4]  && *(ptr+1) == out_frame[5] && sizeof(out_frame) > 4)
		data = check;
